...
/**
 * Класс {@code ToastExpander} предоставляет вспомогательные методы для управления и отображения
 * всплывающих сообщений (toast) и сообщений в виде панели (snackbar) с расширенным функционалом.
 * Он позволяет показывать всплывающие сообщения в течение заданного времени, отображать
 * стилизованные панели и обрабатывать типы сообщений, такие как отладочные (debug) и информационные (info) сообщения.
 *
 * <p>
 * Этот класс использует паттерн "одиночка" (singleton), чтобы обеспечить единую точку управления
 * операциями, связанными с всплывающими сообщениями и панелями. Он также предоставляет методы для
 * отображения отладочных и информационных сообщений, управления их видимостью в зависимости от
 * настроек приложения и удаления дублирующихся строк из сообщений.
 * </p>
 */
